...
def synthetic_experiment(LEARNER, bags, linear='Linear', savefig=False):
    ...
    if linear == 'Linear':
        ...
        plt.title(f"{n} Synthetic Data (Normally Distributed Error)")
        plt.plot(x1, y1, label="Known Function")            
        plt.plot(x1,b1,'.',markersize=15,label='noisy data')
        plt.legend()
        if savefig:
            plt.savefig(f"./normal_figures/{linear}_data_{LEARNER}.png", dpi=300)
        plt.show()
    ...
    if linear == 'Linear':
        ...
        plt.scatter(X_train, 5*X_train, label="Known Function", color='gray', alpha=0.5)
        ...
        plt.legend()
        if savefig:
            plt.savefig(f"./normal_figures/{linear}_train_{LEARNER}.png", dpi=300)
        plt.show()
        ...
        plt.title(f"{bags} Resamples with {conf}% Confidence (Training Data)")
        plt.scatter(X_train, Y_train, label='Data')
        plt.scatter(X_train, np.square(X_train), label="Known Function", color='gray', alpha=0.5)
        plt.scatter(X_train, points, label='Point Predictions', color='red')
        plt.scatter(X_train, lowers, label="Lower Predictions", color = 'orange')
        plt.scatter(X_train, uppers, label='Upper Predictions', color = 'orange')
        plt.legend()
    ...
    if linear == 'Linear':
        ...
        plt.scatter(X_test, 5*X_test, label="Known Function", color='gray', alpha=0.5)
        ...
        plt.legend()
        if savefig:
            plt.savefig(f"./normal_figures/{linear}_test_{LEARNER}.png", dpi=300)
        plt.show()
        ...
        plt.title(f"{bags} Resamples with {conf}% Confidence (Test Data)")
        plt.scatter(X_test, Y_test, label='Data')
        plt.scatter(X_test, np.square(X_test), label="Known Function", color='gray', alpha=0.5)
        plt.scatter(X_test, points, label='Point Predictions', color='red')
        plt.scatter(X_test, lowers, label="Lower Predictions", color = 'orange')
        plt.scatter(X_test, uppers, label='Upper Predictions', color = 'orange')
        plt.legend()
